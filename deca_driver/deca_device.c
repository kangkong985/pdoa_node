...
typedef struct
{
    ...
    uint8       vTempP ;            // IC V temp read during production and stored in OTP (Tmeas @ 23C)
...
int dwt_initialise(int config)
{
    ...
    // Load vBatP and vTempP from OTP
    pdw1000local->vBatP = _dwt_otpread(VBAT_ADDRESS) & 0xff;
    pdw1000local->vTempP = _dwt_otpread(VTEMP_ADDRESS) & 0xff;
 ...
 */
uint8 dwt_gettmeas(void)
{
    return pdw1000local->vTempP;
}
 ...
 * no return value
 */
void dwt_setinterrupt(uint32 bitmask, int enable)
{
    decaIrqStatus_t stat ;
    uint32 mask ;
...
)
{
    ...
    return _dwt_computetxpowersetting(ref_powerreg, (int8)(delta_power / MIXER_GAIN_STEP));
}
